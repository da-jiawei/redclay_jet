...
# plot basic results ----
png("figures/FigS1.calcite_nodules.png", 4.9, 5.1, units = "in", res = 600)
# pdf("figures/FigS1.calcite_nodules.pdf", 4.9, 5.1)
par(mar = c(4, 4, 1, 4))
...
lines(lowess(D47.rs[, 1], D47.rs[, 2], f = 0.2), lwd = 3)
axis(2, 2 + (tix - min(tix)) / diff(range(tix)), tix)
mtext(expression(paste("T"[Delta*"47"]*" (", degree, "C)")), 2, line = 2.5, at = 2.5)
text(2.3, 3, "a", cex=1.2, font=2)

yext = range(d18c$d18O)
tix = seq(floor(min(yext)),
          ceiling(max(yext)), by = 2)
d18c.rs = cbind(d18c$age,
                1 + (d18c$d18O - min(tix)) / diff(range(tix)))
points(d18c.rs[, 1], d18c.rs[, 2], col = "black", bg = site2, pch = 22, cex = 1.2)
axis(4, 1 + (tix - min(tix)) / diff(range(tix)), tix)
mtext(expression(delta^"18"*"O"[c]*" (\u2030)"), 4, line = 2.5, at = 1.5)
text(2.3, 2, "b", cex=1.2, font=2)

dp17 = dp17 %>%
  mutate(Dp17c.low = Dp17c - Dp17c.se2,
         Dp17c.high = Dp17c + Dp17c.se2)
yext = range(dp17$Dp17c.low, dp17$Dp17c.high)
tix = seq(ceiling(min(yext)-28), floor(max(yext+20)), 
          by = 50)
dp17.rs = cbind(dp17$age,
                1 - (dp17$Dp17c - min(tix)) / diff(range(tix)),
                1 - (dp17$Dp17c.low - min(tix)) / diff(range(tix)),
                1 - (dp17$Dp17c.high - min(tix)) / diff(range(tix)))
...
mtext(expression(Delta^"'17"*"O"[c]*" (per meg)"), 2, line = 2.5, at = 0.5)
text(2.3, 1, "c", cex=1.2, font=2)
