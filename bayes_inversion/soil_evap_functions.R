...
soil_evap_model = function(RH_air, T_air, T_air_half_range, time_of_year, d18_p, Dp17_p, Bk_depth){
  # Constants ----
  soil_depth = 30
  pore = .99
  tort = .99
  ...
  vapor_conc_air = (e_air / (Rgas * (atm_T + 273.15))) / 1e6 # mol/cm3
  
  alpha18_eq = alpha_18_l_v(0) # cloud temperature
  ...
  # initial values 
  water_content = rep(initial_water_content, soil_bttm)
  water_content[1] = 0
  ...
  Bk_node = ceiling(Bk_depth / .5 + 1)
  Tsoil.K = soilT[Bk_node] + 273.15
  ...
  d18_c = (d18_sw[Bk_node] + 1e3) * alpha18_c_w_eq - 1e3
  d17_c = (d17_sw[Bk_node] + 1e3) * alpha17_c_w_eq - 1e3
  Dp17_c = delta_prime(d18_c, d17_c)
  D47_c = 0.0391e6 / Tsoil.K ^ 2 + 0.154 # Andersen (2021)
  
  return(results = c(d18_c, Dp17_c, D47_c))
}
