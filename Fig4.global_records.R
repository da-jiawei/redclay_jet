...
## plot ----
png("figures/Fig4.global_records.png", 4.5, 7, units = "in", res = 300)
...
lr04.rs = cbind(lr04$age,
               5 - (lr04$d18O - min(tix)) / diff(range(tix)))
lines(lr04.rs[, 1], lr04.rs[, 2], col = pal[1])
axis(2, 5 - (tix - min(tix)) / diff(range(tix)), tix)
mtext(expression(paste("benthic"*delta^"18"*"O (\u2030)")), 2, line = 2.5, at = 4.5)
...
sw.rs = cbind(sw$age,
              3.6 + (sw$d18sw - min(tix)) / diff(range(tix)),
              3.6 + (sw$d18sw.low - min(tix)) / diff(range(tix)),
              3.6 + (sw$d18sw.high - min(tix)) / diff(range(tix)))
...
axis(4, 3.6 + (tix - min(tix)) / diff(range(tix)), tix)
mtext(expression(paste(delta^"18"*"O"[sw]*" (\u2030)")), 4, line = 2.5, at = 4.2)
yext = range(ob.am$ob.am)
tix = seq(floor(min(yext*10)), 
          ceiling(max(yext*10+3)), by = 5)/10
ob.am.rs = cbind(ob.am$age,
...
dsst.rs = cbind(DSST$age,
                2.5 - (DSST$TH - min(tix)) / diff(range(tix)))
lines(dsst.rs[, 1], dsst.rs[, 2], col = pal[5], lwd = 3)
axis(4, 2.5 - (tix - min(tix)) / diff(range(tix)), tix)
mtext(expression(paste(Delta*"SST (", degree, "C)")), 4, line = 2.5, at = 2)
